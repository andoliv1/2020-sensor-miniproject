...
if __name__ == "__main__":
    ...
    print("This is the time mean " + str(time_np.mean().total_seconds()))
    print("This is the time var " + str(pow(np.std(time_np).total_seconds(),2)))
    ...
    for k in data:
        ...
        print("This is the median before removed anomalies " + str(data[k]["office"].median()) + " for " + str(k) + " at office")
        print("This is the variance before removed anomalies " + str(data[k]["office"].var()) + " for " + str(k) + " at office")
        #density function of each sensor type which is letter c) of task 2
        data[k]["office"].to_frame().plot.density()
        plt.xlabel(k)
        plt.ylabel("Probability")
        plt.title("PDF plot of " + str(k))
        plt.savefig("PDF_" + str(k) + ".png")

        data[k]["office"].to_frame().hist()
        plt.xlabel(k)
        plt.ylabel("Quantity")
